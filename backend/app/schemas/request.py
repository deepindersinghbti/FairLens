...
class AnalyzeBiasRequest(BaseModel):
    ...
    def validate_columns(self) -> "AnalyzeBiasRequest":
        ...
        self.prediction_column = self._normalize_optional_prediction(self.prediction_column)

        if self.target_column.lower() == self.sensitive_attribute.lower():
            raise ValueError("Target column and sensitive attribute must be different")
        ...
        if self.prediction_column is not None:
            ...
            if prediction_lower == self.target_column.lower():
                raise ValueError("Prediction column and target column must be different")
